...
def traverse_json(data, NoMulti, start_key=None):
    ...
    if start_key:
        try:
            ...
            print(f"\n{Fore.WHITE}Starting at key: {path}")
            current = value
    ...
    while isinstance(current, (dict, list)):
        if isinstance(current, dict):
            print(f"\n{Fore.MAGENTA}Keys available:")
            keys = list(current.keys())
            for i, key in enumerate(keys):
                print(f"{i+1}: {' ' if i < 9 else ''}{Fore.LIGHTMAGENTA_EX}{key}")
                    ...
                    if results:
                        ...
                        for i, (path, value) in enumerate(results):
                            ...
                            if NoMulti and value_str == '[Nested structure]':
                                ...
                                d += 1
                                print(f"{d}. {Fore.LIGHTMAGENTA_EX}Path: {path}, Value: {value_str}")
...
def games_game_pre(game_pre, selected_folder, mode="games"):
    ...
    if not folders:
        ...
        while True:
            ...
            for idx, folder in enumerate(folders):
                print(f"{idx + 1}: {Fore.LIGHTMAGENTA_EX}{folder}{Style.RESET_ALL}")
...
if __name__ == "__main__":
    ...
    print(f'{Fore.MAGENTA}Discord: ho0key{Style.RESET_ALL}')
    while True:
        game_pre = ""
        run_option = get_valid_input(f"\nFleasionPF+\n"
                           f"0: {Fore.LIGHTMAGENTA_EX}Custom{Style.RESET_ALL}\n"
                           f"1: {Fore.LIGHTMAGENTA_EX}Games{Style.RESET_ALL}\n"
                           f"2: {Fore.LIGHTMAGENTA_EX}PF{Style.RESET_ALL}\n"
                           f"3: {Fore.LIGHTMAGENTA_EX}Presets{Style.RESET_ALL}\n"
                           f"4: {Fore.LIGHTMAGENTA_EX}Previewer{Style.RESET_ALL}\n"
                           f"5: {Fore.LIGHTMAGENTA_EX}Blocker{Style.RESET_ALL}\n"
                           f"6: {Fore.LIGHTMAGENTA_EX}Cache Settings{Style.RESET_ALL}\n"
                           f"7: {Fore.LIGHTMAGENTA_EX}Fleasion Settings{Style.RESET_ALL}\n"
                           f"8: {Fore.LIGHTMAGENTA_EX}Credits{Style.RESET_ALL}\n"
        ...
        try:
            match run_option:
                case 0:
                    while True:
                        custom_option = get_valid_input(
                            f"\nEnter custom option:\n"
                            f"1: {Fore.BLUE}From Fleasion Assets{Style.RESET_ALL}\n"
                            f"2: {Fore.BLUE}From Roblox Temp{Style.RESET_ALL}\n"
                ...
                case 2:
                    clear_terminal()
                    game_pre = f"assets/games/Phantom forces/"
                    json_file_path = f"{game_pre}assets.json"
                    game_code = f"{game_pre}code.py"
                ...
                case 3:
                    ...
                    while True:
                        preset_option = get_valid_input(
                                        ...
                                        f"1: {Fore.CYAN}Load Preset{Style.RESET_ALL}\n"
                                        f"2: {Fore.CYAN}Create Preset{Style.RESET_ALL}\n"
                                        f"3: {Fore.CYAN}Edit Preset{Style.RESET_ALL}\n"
                                        f"Type 'back' to return to the main menu.\n: ",
                                        valid_values=[1,2,3],
                                        top = False
                        )
